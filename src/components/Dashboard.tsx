...
export default function Dashboard() {
  ...
  const [pendingPlatform, setPendingPlatform] = useState<'grab' | 'bolt' | 'vip' | undefined>(undefined);
  ...
  // (The fuel-price *history* shown on Analytics is now collected by a
  // separate cron Worker writing to D1, independent of app traffic — no
  // client-side snapshot trigger needed here anymore.)
  useEffect(() => {
    prefetchFuelPrices();
    ...
    <>
      ...
      {showTripTimer && (
        <TripTimerDialog
          ...
          onEndTrip={(duration, startTime, platform) => {
      ...
      {showAddEntry && (
        <AddEntryModal
          ...
          lockedPlatform={pendingPlatform}
          onSave={(entry) => {
            addEntry(entry);
  ...
  const header = (
    <div className="flex items-center justify-between">
      ...
      {/* How to use this app — guide panel */}
      <button
